...
if(isset($_REQUEST['st_id']))
{
	...
	$obj=new control();
	$ss = $obj->stsig($st_id);
	$fe_st = mysql_fetch_array($ss);
	if($fe_st['status']=='Enabled')
	{
		$obj=new control();
		$st = $obj->sta($st_id);
		header("location:vuser.php");
...
<div id="main_container">
	...
	*/
	...
	$tbl_name="user_reg";		//your table name
	...
	*/
	...
	$targetpage = "vuser.php"; 	//your file name  (the name of this file)
	...
	$sql = "select $tbl_name.*,country.cname,state.sname,city.ctname from $tbl_name join country on user_reg.country=country.cid join state on $tbl_name.state=state.sid join city on $tbl_name.city=city.ctid LIMIT $start, $limit";
...
<?php
		...
		<tr>
			<th>Status</th>
			<td><a href="vuser.php?st_id=<?php echo $r['user_id']; ?>"><?php echo $r['status']; ?></a></td>
		</tr>			
		<tr>
			<th>Action</th>
			<td><a href="vuser.php?del_id=<?php echo $r['user_id']; ?>"><img src="images/trash.png" alt="delete" title="delete" border="0" />Delete</a></td>
		</tr>			
	</table>
